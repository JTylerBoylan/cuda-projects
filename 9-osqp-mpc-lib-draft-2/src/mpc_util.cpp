...
namespace orlqp
{
    ...
    QPProblem::Ptr mpc2qp(const MPCProblem::Ptr mpc)
    {
        ...
        to_hessian(n, qp->hessian, Nx, Nu, Nn, mpc->state_objective, mpc->control_objective);
        to_gradient(n, qp->gradient, Nx, Nu, Nn, mpc->state_objective, mpc->xf);
        to_linear_constraint(n, m, qp->linear_constraint, Nx, Nu, Nn, mpc->state_dynamics, mpc->control_dynamics);
        to_lower_bound(m, qp->lower_bound, Nx, Nu, Nn, mpc->x0, mpc->x_min, mpc->x_max, mpc->u_min, mpc->u_max);
        to_upper_bound(m, qp->upper_bound, Nx, Nu, Nn, mpc->x0, mpc->x_min, mpc->x_max, mpc->u_min, mpc->u_max);
        return qp;
    }
